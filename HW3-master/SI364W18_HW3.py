## SI 364 - Winter 2018
## HW 3
## SALVATORE DIGOIA -- WORKED WITH ALEX SHELL + FRANKY ANTENUCCI
...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/sfdigi_hw3"
## Provided:
...
class TweetForm(FlaskForm):
    text = StringField('Enter the text of the tweet (280 characters max.):', validators = [Required(),Length(1,280)])
    username = StringField('Enter the username of the twitter user (no "@"', validators = [Required(), Length(1, 64)])

    def validate_username(self, field):
        if field.data[0] == '@':
            raise ValidationError('Username may NOT start with @ symbol, try again without it!')
    ...
    def validate_display_name(self, field):
        if len(field.data.split()) < 2:
            raise ValidationError('Display name is not at least two words long, try again!')
...
def index():
    ...
    if form.validate_on_submit():
        ...
        if tweet:
            flash('This tweet has already been saved!')
            return redirect(url_for('see_all_tweets'))

        else:
            tweet = Tweet(text = text, user_id = user.user_id)
            db.session.add(tweet)
            db.session.commit()
            flash('This tweet has been successfully added to the database!')
...
def see_all_users():
    ...
    return render_template('all_users.html', users=db_users)
...
def get_longest_tweet():
    ...
    for tweet in db_tweets:
        clean_tweet = tweet.text.replace(' ', '')
        user_table = User.query.filter_by(user_id=tweet.user_id).all()
        for field in user_table:
            user = field.username
